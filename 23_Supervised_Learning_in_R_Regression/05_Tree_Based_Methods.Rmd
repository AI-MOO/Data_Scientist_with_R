...
-   Fill in the blanks to get the number of trees with the minimum value of the columns `train_rmse_mean` and `test_rmse_mean`.
    ...
    -   [**`which.min()`**](https://www.rdocumentation.org/packages/base/topics/which.min) returns the index of the minimum value in a vector.
...
-   Now call [**`predict()`**](https://www.rdocumentation.org/packages/xgboost/topics/predict.xgb.Booster) on `bikesAugust.treat` to predict the number of bikes rented in August.
